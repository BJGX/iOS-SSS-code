...
fragment float4 liquidGlassFragment(VertexOut in               [[stage_in]],
    ...
    float3 tinted = mixed * (1.0 - u.tintAlpha) + u.tintColor * u.tintAlpha;
    // 边缘高光
    tinted += clamp(highlight(normalizedInside) * pow(edgeBlendFactor, 5.0), 0.0, 1.0) * 0.5;
    ...
    mixed = mix(tinted, veilTint, 0.1 * weight);
...
//    float edgeFactor = smoothstep(0.0, 1.0, 1.0 - clamp(sdf, 0.0, 1.0));
//    float3 final = mix(baseTex, mixed, edgeFactor);
