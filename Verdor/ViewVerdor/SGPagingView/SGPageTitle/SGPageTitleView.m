...
- (instancetype)initWithFrame:(CGRect)frame delegate:(id<SGPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames configure:(SGPageTitleViewConfigure *)configure {
    if (self = [super initWithFrame:frame]) {
        ...
        if (configure.indicatorStyle == SGIndicatorStyleFixedSpace) {
...
- (void)YH__changeSelectedStateButton:(NSInteger)index
{
    if (index < self.btnMArr.count && index >= 0) {
        [self P_changeSelectedButton:self.btnMArr[index]];
    }
}
...
- (void)setupSubviews {
    ...
    if (self.configure.indicatorStyle == SGIndicatorStyleFixedSpace && self.configure.indicatorFixedImage) {
        
        
        CGSize imageSize = self.configure.indicatorFixedImage.size;
        CGRect frame = CGRectMake(-20, self.frame.size.height - imageSize.height, ScreenWidth, imageSize.height);
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:frame];
        imageView.image = self.configure.indicatorFixedImage;
...
- (void)layoutSubviews {
    ...
    if (_selectedIndex > self.btnMArr.count - 1) {
        return;
    }
    // 布局子控制器的 frame
...
- (void)P_layoutSubviews {
    ...
    if (self.configure.indicatorStyle == SGIndicatorStyleFixedSpace && self.configure.itemWidth == 0) {
        CGFloat addX = self.configure.normalLeftSpace;
        __block CGFloat btnX = addX;
        CGFloat btnY = 0;
        CGFloat btnH = selfHeight - self.configure.indicatorHeight;
        [self.btnMArr enumerateObjectsUsingBlock:^(UIButton * obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            btnW = tempSize.width + self.configure.titleAdditionalWidth;
            obj.frame = CGRectMake(btnX, btnY, btnW, btnH);
            btnX = btnX + btnW + self.configure.itemSpace;
        }];
        UIButton *lastBtn = self.btnMArr.lastObject;
        CGFloat scrollViewWidth = CGRectGetMaxX(lastBtn.frame) + addX;
    ...
    else if (self.allBtnWidth <= self.bounds.size.width) { // SGPageTitleView 静止样式
        ...
        else if (self.configure.indicatorStyle == SGIndicatorStyleFixedSpace && self.configure.itemWidth > 0) {
            btnH = selfHeight;
            btnW = self.configure.itemWidth;
        }
        else {
            btnH = selfHeight;
        }
        ...
        if (self.configure.indicatorStyle == SGIndicatorStyleDefault) {
            ...
            btnH = self.configure.itemWidth;
            btnY = (selfHeight - btnH) / 2.0;
        }
        else {
            btnH = selfHeight;
        }
        [self.btnMArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            CGSize tempSize = [self P_sizeWithString:self.titleArr[idx] font:self.configure.titleFont];
            if (self.configure.indicatorStyle == SGIndicatorStyleFixedSpace) {
                btnW = tempSize.width > self.configure.itemWidth - 5 ? tempSize.width + 5 : self.configure.itemWidth;
                if (idx != 0) {
                    btnX += self.configure.titleAdditionalWidth;
                }
            }
            else{
...
- (void)setupTitleButtons {
    ...
    if (self.configure.normalLeftSpace > 0) {
        self.allBtnWidth += self.configure.normalLeftSpace * 2;
    }
    
    if (self.configure.itemSpace > 0) {
        self.allBtnWidth += self.configure.itemSpace * (titleCount -1);
    }
    ...
    if (self.allBtnWidth <= self.bounds.size.width) { // SGPageTitleView 静止样式
        for (NSInteger index = 0; index < titleCount; index++) {
            ...
            if (self.configure.itemCornerRadius > 0) {
                btn.layer.cornerRadius = self.configure.itemCornerRadius;
                btn.layer.masksToBounds = YES;
            }
//            if (self.configure.indicatorStyle == SGIndicatorStyleFixedSpace) {
//                btn.layer.cornerRadius = self.configure.itemWidth /2.0;
//                btn.layer.masksToBounds = YES;
//                btn.backgroundColor = self.configure.titleBackgroundColor;
//            }
        }
...
//            btn.backgroundColor = self.configure.titleBackgroundColor;
//
//            if ([self.delegatePageTitleView respondsToSelector:@selector(pageTitleView:setButtonState:index:)]) {
//                [self.delegatePageTitleView pageTitleView:self setButtonState:btn index:idx];
//            }
//
//        }];
...
//        button.selected = YES;
//        button.backgroundColor = self.configure.titleSelectedBackgroundColor;
//
//        return;
//    }
//
...
- (void)reloadScrollViewTitle:(UIScrollView *)scrollView
{
    return;
    if (self.configure.indicatorStyle != SGIndicatorStyleFixedSpace) {
        return;
    ...
    int x = scrollView.contentOffset.x + self.frame.size.width / 2.0;
    NSInteger index = 0;
    CGFloat btnX = 0;
    for (int i = 0; i < self.btnMArr.count; i++) {
        UIButton *btn = self.btnMArr[i];
        btnX = CGRectGetMinX(btn.frame);
        index = i;
        if (btnX >= x) {
    ...
    if (index > 0) {
        UIButton *btn = self.btnMArr[index - 1];
        CGFloat lastX = CGRectGetMaxX(btn.frame);
        CGFloat y = x - lastX;
        if (y < self.configure.titleAdditionalWidth / 2.0) {
            index -= 1;
        }
    }
...
{
    if (!decelerate) {
        [self reloadScrollViewTitle:scrollView];
    }
}
...
- (void)setSelectedIndex:(NSInteger)selectedIndex
{
    ...
    UIButton *button = self.btnMArr[selectedIndex];
...
- (void)addTitles:(NSArray *)titles
{
    ...
    NSMutableArray *newArray = [[NSMutableArray alloc] initWithArray:self.titleArr];
    NSInteger originCount = self.titleArr.count;
    [newArray addObjectsFromArray:titles];
    [self addBtns:titles originCount:originCount];
    self.titleArr = newArray;
    
    
    [self resetLayoutSubviews:originCount];
...
- (void)resetLayoutSubviews:(NSInteger)originCount
{
    ...
    else if (self.configure.indicatorStyle == SGIndicatorStyleFixedSpace) {
        btnH = self.configure.itemWidth;
        btnY = (selfHeight - btnH) / 2.0;
    }
    else {
    ...
    [self.btnMArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (idx >= originCount) {
            ...
            CGSize tempSize = [self P_sizeWithString:self.titleArr[idx] font:self.configure.titleFont];
            if (self.configure.indicatorStyle == SGIndicatorStyleFixedSpace) {
                btnW = tempSize.width > self.configure.itemWidth - 5 ? tempSize.width + 10 : self.configure.itemWidth;
                if (idx != 0) {
                    btnX += self.configure.titleAdditionalWidth;
                }
            }
...
- (void)addBtns:(NSArray *)titles originCount:(NSInteger)originCount
{
