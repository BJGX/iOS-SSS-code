...
- (instancetype)initWithSkipType:(SkipType)skipType{
    ...
    if (self) {
        ...
        _skipType = skipType;
        CGFloat y = SafeTopSpace > 0 ? 44 : 20;
        //环形
...
-(UILabel *)timeLab{
    if(_timeLab ==  nil){
        ...
        _timeLab.textColor = kOrangeColor;
...
- (void)setTitleWithSkipType:(SkipType)skipType duration:(NSInteger)duration{
    ...
    switch (skipType) {
        ...
        case SkipTypeTime:{
            self.hidden = NO;
            self.timeLab.text = [NSString stringWithFormat:@"%@ %@", [YQUtils formatStringWithInteger:duration], DurationUnit];
        }
            break;
        case SkipTypeText:{
            self.hidden = NO;
        ...
        case SkipTypeTimeText:{
            self.hidden = NO;
            self.timeLab.text = [NSString stringWithFormat:@"%@ %@", [YQUtils formatStringWithInteger:duration], SkipTitle];
        }
            break;
        case SkipTypeRoundTime:{
            self.hidden = NO;
...
-(void)setLeftRightSpace:(CGFloat)leftRightSpace{
    ...
    frame = CGRectMake(leftRightSpace, frame.origin.y + PUB_NAVBAR_OFFSET, width-2*leftRightSpace, frame.size.height);
...
-(void)setTopBottomSpace:(CGFloat)topBottomSpace{
    ...
    frame = CGRectMake(frame.origin.x, topBottomSpace + PUB_NAVBAR_OFFSET / 2.0, frame.size.width, height-2*topBottomSpace);
    self.timeLab.frame = frame;
   [self cornerRadiusWithView:self.timeLab];
}
