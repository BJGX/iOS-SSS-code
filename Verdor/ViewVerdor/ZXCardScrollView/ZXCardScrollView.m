...
+ (instancetype)cardScrollViewWithFrame:(CGRect)frame
{
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView{
    ...
    if ([self.delegate respondsToSelector:@selector(cardScrollView:scrollViewToIndex:)]) {
        NSInteger index = scrollView.contentOffset.x / (self.itemWidth + self.itemSpace);
        [self.delegate cardScrollView:self scrollViewToIndex:index];
    }
...
//        [UIImage imageWithContentsOfFile:imagePath];
        [cell.imageView sd_setImageWithURL:imageOfNSURL(imagePath) placeholderImage:self.placeholderImage];
    }
    else
    {
...
-(void)setImgArr:(NSArray *)imgArr{
    ...
    if (self.infiniteLoop) {
        [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForRow:imgArr.count / 2 * 100 inSection:0] atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:NO];
    }
...
- (void)setSelectedIndex:(NSInteger)selectedIndex
{
    ...
    if (selectedIndex < self.totalItems) {
...
- (void)setImageGroups:(NSArray<NSString *> *)imageGroups
{
    self.imgArr = imageGroups;
}
