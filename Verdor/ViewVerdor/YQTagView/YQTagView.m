...
- (instancetype)initWithName:(NSString *)name font:(UIFont *)font showSeletedImage:(BOOL)showSeletedImage;
...
- (instancetype)initWithName:(NSString *)name font:(UIFont *)font showSeletedImage:(BOOL)showSeletedImage {
    if (self = [super init]) {
        _name = name;
        _showSeletedImage = showSeletedImage;
        self.font = font;
    }
    return self;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame: frame]) {
        ...
        self.leftImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 16, 16)];
        self.leftImageView.image = [UIImage imageNamed:@"icon_f_unse"];
        self.leftImageView.highlightedImage = [UIImage imageNamed:@"icon_f_se"];
...
- (void)layoutSubviews {
    ...
    self.leftImageView.hidden = !self.tagModel.showSeletedImage;
    self.leftImageView.frame = CGRectMake(x, 0, 16, 16);
    self.leftImageView.centerY = self.contentView.center.y;
    self.leftImageView.highlighted = self.tagModel.selected;
    
    x = self.tagModel.showSeletedImage ? x + 24 : x;
    width = self.tagModel.showSeletedImage ? width - 24 : width;
    CGRect frame = CGRectMake(x, 0, width, [self.tagModel contentSize].height);
    self.tagLabel.frame = frame;
    self.tagLabel.centerY = self.contentView.center.y;
    
}

@end

@interface FMEqualSpaceFlowLayout2 : UICollectionViewFlowLayout
...
@implementation FMEqualSpaceFlowLayout2
...
- (void)prepareLayout
{
    ...
    for (NSInteger idx = 0; idx < itemCount; idx++) {
        ...
        if (xNextOffset - minimumInteritemSpacing > colloctionViewBounds.size.width - sectionInset.right && self.showHorizontal == NO) {
        ...
        self.contentWidth = CGRectGetMaxX(layoutAttributes.frame)+sectionInset.right;
    }
...
- (CGSize)collectionViewContentSize {
    ...
    if (self.showHorizontal) {
        contentSize  = CGSizeMake(self.contentWidth, self.contentHeight);
    }
...
@interface YQTagView ()<UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@property (nonatomic, strong) FMEqualSpaceFlowLayout2 *flowLayout;
...
- (void)commonInit {
    ...
    _tagKey = @"";
    _tagSelectedBorderWidth = 0;
    _tagSelectedBorderColor = [UIColor clearColor];
    
    _tagHeight = 28;
    _mininumTagWidth = 0;
    _maximumTagWidth = CGFLOAT_MAX;
    _lineSpacing = 10;
    _interitemSpacing = 15;
...
- (CGSize)intrinsicContentSize {
    ...
    self.viewHeight = contentSize.height;
...
- (void)setTagsArray:(NSArray<NSString *> *)tagsArray {
    ...
    [tagsArray enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        FMTagModel *tagModel = [[FMTagModel alloc] initWithName:obj font:self.tagFont showSeletedImage:self.showSeletedImage];
...
- (void)addTag:(NSString *)tagName {
    ...
    FMTagModel *tagModel = [[FMTagModel alloc] initWithName:tagName font:self.tagFont showSeletedImage:self.showSeletedImage];
    [self.tagModels addObject:tagModel];
    [self.collectionView reloadData];
    [self invalidateIntrinsicContentSize];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    FMTagModel *tagModel = self.tagModels[indexPath.row];
    if ([tagModel.name isEqualToString:self.tagKey]) {
...
- (void)setCell:(FMTagCell *)cell selected:(BOOL)selected {
    ...
    cell.leftImageView.highlighted = selected;
    if (selected) {
        ...
        cell.layer.borderWidth = self.tagSelectedBorderWidth;
...
- (void)collectionView:(UICollectionView *)collectionView didDeselectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.unDidClickTag) {
        self.unDidClickTag(indexPath.row);
    }
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.allowsMultipleSelection) {
        ...
        if (self.didClickTag) {
            self.didClickTag(indexPath.row);
        }
        return;
    }
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        FMEqualSpaceFlowLayout2 *flowLayout = [[FMEqualSpaceFlowLayout2 alloc] init];
        flowLayout.delegate = self;
        self.flowLayout = flowLayout;
        flowLayout.showHorizontal = self.showHorizontal;
...
{
    _showHorizontal = showHorizontal;
    self.flowLayout.showHorizontal = showHorizontal;
    self.collectionView.scrollEnabled = showHorizontal;
}
...
-(CGFloat)tagSelectedBorderWidth {
    if (!_tagSelectedBorderWidth) {
        return _tagSelectedBorderWidth;
    }
    return _tagSelectedBorderWidth;
}
