...
@interface VFBindMoblieVC ()<SGPageTitleViewDelegate>
...
@implementation VFBindMoblieVC

- (void)viewDidLoad {
    [super viewDidLoad];
    NSString *title = self.isBind ? @"绑定" : @"注册";
    self.navTitle = title;
    [self.bindBtn setTitle:title.localized forState:0];
    [self setTitleViewUI];
    [self.phoneTF setPlaceholderColor];
    [self.codeTF setPlaceholderColor];
    [self.passwordTF setPlaceholderColor];
    [self.uidTF setPlaceholderColor];
    ...
    [self.passwordTF yq_addTextFieldChangedAction:^(UITextField * _Nullable textField) {
...
- (IBAction)loginAction:(UIButton *)sender {
    ...
    if (self.pageTitleView.signBtnIndex == 1) {
        ...
        NSString *uid = self.uidTF.text.length > 0 ? self.uidTF.text : @"";
        
        NSString *url = @"api/en/user/registerByEmail";
        NSDictionary *dic = @{@"mail":self.phoneTF.text,
                              @"password":self.passwordTF.text,
                              @"code":self.codeTF.text,
                              @"invite":uid
        };

        NSString *tips = self.isBind ? @"正在绑定": @"正在注册";
...
//                    [VFTipsView showView:@"绑定成功" content:@"绑定后需要重新登录" leftBtn:nil rightBtn:@"确定" block:^(NSInteger code) {
//                        if (code == 1) {
//                            [YQUserModel shared].user.tourist = 0;
//                            [YQUserModel shared].tourist = 2;
////                            [YQUserModel saveUserInfo];
//
//                        }
//                    }];
    ...
    NSString *url = @"api/en/user/register";
...
- (IBAction)sendAction:(UIButton *)sender {
    ...
    if (self.pageTitleView.signBtnIndex == 1) {
        ...
        NSDictionary *dic = @{@"mail":self.phoneTF.text, @"type":@1};
    ...
    //phone 手机号.
    //type 1：注册；2：登录. 3：更换手机 4：找回密码 5: 发送验证码
    NSDictionary *dic = @{@"phone":self.phoneTF.text, @"type":@1,@"area_code":self.phoneTF.selectedDialCode};
    [YQNetwork requestMode:POST tailUrl:url params:dic showLoadString:@"正在发送" andBlock:^(id obj, NSInteger code) {
        if (code == 1) {
            [sender startCountDown:60];
        }
...
- (IBAction)backAction:(UIButton *)sender {
    if (self.isBind) {
        UIViewController *vc = [YQUtils returnStoryboardVC:@"Mine" vcName:@"VFLoginVC"];
...
//    self.numberTF.text = @"";
    ...
    if (selectedIndex == 1) {
        self.phoneTF.placeholder = @"请输入邮箱".localized;
