...
//    [self.progressBackView addLiquidGlassView:100 shadowIntensity:0.6 highlightIntensity:0.6];
    ...
    [[FCVPNManager sharedManager] removeManager];
...
- (void)saveProxyData
{
    ...
    NSString *host = [FCSocket getIp:model.host];
    [YQUserModel shared].chooseModel.ip = model.host;
    ...
    NSDictionary *param = @{
        ...
        @"ot_path": model.ot_path.length > 0 ? [NSString stringWithFormat:@"/%@/",model.ot_path] : @"",
        @"ota": @(model.ota),
    ...
    [[FCVPNManager sharedManager] setDefaultConfigGroupWithParam:param proxy:self.isGlobal];
}
...
//    }];
    ...
    [[FCVPNManager sharedManager] switchVPN];
