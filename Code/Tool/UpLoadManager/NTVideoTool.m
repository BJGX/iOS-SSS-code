...
#import "NTVideoTool.h"
#import <Photos/Photos.h>

@implementation NTVideoTool
...
+ (void)convertMovToMp4FromAVURLAsset:(AVURLAsset*)urlAsset
  ...
  // 查询是否有匹配的预设
    if ([compatiblePresets containsObject:[self getAVAssetExportPresetQuality:exportQuality]]) {
        ...
        NSString *resultPath;
        if ([avAsset.URL.path length]) {
            NSString *name = [[avAsset.URL.path lastPathComponent] stringByDeletingPathExtension];
            resultPath = [path stringByAppendingFormat:@"/%@.mp4",name];
            
            
            if ([fileManager fileExistsAtPath:resultPath]) {
                NSURL *FileUrl = [NSURL fileURLWithPath:resultPath];
                NSData *mp4Data = [NSData dataWithContentsOfURL:FileUrl];
                movEncodeToMpegToolResultBlock(FileUrl, mp4Data , nil);
                return;
            }
        }
        else{
            ...
            resultPath = [path stringByAppendingFormat:@"/%@.mp4",dateStr];
