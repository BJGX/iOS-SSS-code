...
@property (nonatomic, strong) CAShapeLayer *secondProgressLayer;
...
- (CAShapeLayer *)secondProgressLayer {
    if (!_secondProgressLayer) {
        _secondProgressLayer = [CAShapeLayer layer];
        _secondProgressLayer.frame = CGRectMake((ZZCircleSelfWidth-_realWidth)/2.0, (ZZCircleSelfHeight-_realWidth)/2.0, _realWidth, _realWidth);
        
        _secondProgressLayer.fillColor = [UIColor clearColor].CGColor;//填充色
        _secondProgressLayer.lineWidth = _strokeWidth;
        _secondProgressLayer.strokeColor = _pathFillColor.CGColor;
        _secondProgressLayer.lineCap = @"round";
        
        UIBezierPath *circlePath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(_realWidth/2.0, _realWidth/2.0) radius:_realWidth/2.0 startAngle:_startAngle endAngle:(2*M_PI-_reduceAngle+_startAngle) clockwise:YES];
        _secondProgressLayer.path = circlePath.CGPath;
        _secondProgressLayer.strokeEnd = 0.0;
    }
    return _secondProgressLayer;
}
...
- (void)setSecondPathFillColor:(UIColor *)secondPathFillColor
{
    if (_secondPathFillColor != secondPathFillColor) {
        _secondPathFillColor = secondPathFillColor;
        self.secondProgressLayer.strokeColor = secondPathFillColor.CGColor;
    }
}
...
- (void)setSecondProgress:(CGFloat)secondProgress
{
    //准备好显示
    self.prepareToShow = YES;
...
- (void)startSecondAnimation
{
    ...
    pathAnima.toValue = [NSNumber numberWithFloat:_secondProgress];
    pathAnima.fillMode = kCAFillModeForwards;
    pathAnima.removedOnCompletion = NO;
    [self.secondProgressLayer addAnimation:pathAnima forKey:@"strokeEndAnimation2"];
}
...
- (void)showView
{
    if (_progress > _secondProgress) {
        [self.layer insertSublayer:self.progressLayer above:self.secondProgressLayer];
    }
    else{
        [self.layer insertSublayer:self.progressLayer below:self.secondProgressLayer];
    }
}
